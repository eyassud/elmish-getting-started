  ...
  Html.ul [
    prop.children [
      ...
      for todo in todoList do
          for todoBeingEdited in state.TodoBeingEdited do
            if todoBeingEdited.Id = todo.Id then (renderEditForm todoBeingEdited dispatch)
            else (renderTodo todo dispatch)    
    ]
  ]
